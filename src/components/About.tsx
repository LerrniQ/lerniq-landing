...
export default function About() {
  ...
  return (
    <section id="about" ref={ref} className="scroll-reveal py-24 px-[5%] bg-brand-dark-blue/30">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {CARDS.map(({ Icon, iconBg, iconColor, title, text }) => (
            <div
              key={title}
              className="bg-white/[0.03] border border-white/10 rounded-3xl p-10 transition-all duration-300 hover:-translate-y-2 hover:bg-white/5 hover:border-brand-blue hover:shadow-[0_20px_60px_rgba(46,63,226,0.2)]"
            >
              <div className={`w-16 h-16 ${iconBg} rounded-2xl flex items-center justify-center mb-6`}>
                <Icon size={28} className={iconColor} />
              </div>
              <h3 className="text-2xl font-semibold mb-4">{title}</h3>
              <p className="text-white/70 leading-[1.8]">{text}</p>
            </div>
          ))}
        </div>
      </div>
